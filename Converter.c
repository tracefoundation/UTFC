...
WriteBackChinese(void) //GB2312
{
	...
	for(i=0; i<nChinese; i++)
	{
		...
		else if (ConversionDirection == OthersToUnicode &&
			SourceEncoding != NS &&
			SourceFileFormat != UnicodeTXT)
		{
			if(GB2Unicode[ch] != 0) ch = GB2Unicode[ch];//Convert from CP936 (GB2312) to Unicode
		}
		else;//No conversion.
...
NEWFONT:	//Start to collect font info of a new font in fonttbl.
	...
	for(i=0; fontBuffer[i] != '\0'; i++)
	{
		...
		{
			if(fontBuffer[i+1]=='\\') continue;//skip this space.
...
//-----------------------------set up target fonts-----------------------------------
		else if(ch == '}')//The last '}' of fontinfo header
		{
			...
			{	//Write target font info
				...
				{
					fputs("{\\f", targetFile); // {\f
					fprintf(targetFile, "%d", sourceFontIDs[0]); //Output fontID represented as string
					fputc(0x20, targetFile);
					fputs(TibetanFontNames[BaseIndex[TargetEncoding]], targetFile);
					fputs(";}", targetFile);
					//Remeber the target font info here
...
int parseRTFforRTF2RTF()
{
	...
	header = No;//Don't process control words in header  //20080229
	nTibetan = 0;
	nLatin = 0;
	nChinese = 0;
	nFormatBuffer = 0;
	...
	while((ch = getc(sourceFile)) != EOF)
	{
			...
			if(retInfo == ControlWord)	//It's control word.
			{
				if(!strcmp(szKeyword, "u"))
				{
					...
					if(currentFontInfo.isTibetan == Yes)//20090615
					{
						if(SourceEncoding == Unicode)//Only apply to Unicode 2 others
						{
							//if(param>=0xf40 && param<=0xfbc)//Tibetan constants or vowels
							if(param != 0x0f0d)//Tibetan constants or vowels
							{
								if(nFormatBuffer>0 && holdTibetanBuffer == No)//Format buffer is not empty, write back them
								{
									if(nTibetan>0) TranslateAndWriteBack();
						...
						{
							if(SourceEncoding == Ty || SourceEncoding == Bzd) continue;//ignore \u32379 in \u32379\'c0\'5d
		...
		default://text:  characters //20080229
			...
			{
				if(currentFontInfo.isTibetan == Yes && ch > 0x20 && ch <= 0xff) //TSL Tibetan
				{
					AddCharacterToSourceTextBuffer((int) ch);
				}
...
//						if(param>=0xf40 && param<=0xfbc) holdTibetanBuffer = Yes; //Hold on!!
						if(param != 0xf0d) holdTibetanBuffer = Yes; //Hold on!!
				...
				else if(!strcmp(szKeyword, "atrfstart") ||
				...
				else if(!strcmp(szKeyword, "shppict") || !strcmp(szKeyword, "nonshppict")) //image or picture appears   //20090218  whole block was added.
				{
					if(nTibetan>0) TranslateAndWriteBack();
					if(nLatin>0) WriteBackLatin();
					if(nChinese>0) WriteBackChinese();
...
int IsSanskritVowel(char *thecomponent)
{
	...
	{
		if( !strcmp(thecomponent, "a") ||
//			!strcmp(thecomponent, "'I") ||  //20080620
//			!strcmp(thecomponent, "'U") ||  //20080620
//			!strcmp(thecomponent, "I")||    //20080620
			!strcmp(thecomponent, "'i")||
...
//				WriteBackUnicode(0xf60);//'
//				WriteBackUnicode(0xf72);//vowel i
			}
...
//			if((ch == '\n' || ch == 0x0d) && TargetFileFormat != UnicodeTXT) fputc('\n', targetFile);
		}
		else if((j==0)&&(ch>0x7f)) { thecode=ch; j=1; continue; }   //20090204
/*		else if((j==1)&&(ch>=0x7f)) //Chinese, include those instructions, take off them   //20090204 //20090918
		{
			//20080918
			if(thecode == 0xa1 && ch == 0xbc) { instruction = 1; j = 0;}//The Chinese instructions
			else if (thecode == 0xa1 && ch == 0xbd) { instruction = 0; j = 0;}
			else if (thecode == 0xaa && ch == 0xa4) { instruction = 0; j = 0;}
			else if (thecode == 0xa1 && ch == 0xfe)
			{
				ch = 0x20;
			...
			{
				if(instruction==1) j=0;//take off instructions
				else //Chinese
				{
					if(nTibetan>0) TranslateAndWriteBack();
					if(nLatin>0) WriteBackLatin();
