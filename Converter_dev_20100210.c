 ...
 char *sourceEncoding, char *targetEncoding, char *TFF)  //200802
{
    ...
    targetFile = fopen(targetFileName, "wb"); //20081124
    if (!targetFile) return NoTargetFile;    
	...
	if(ConversionDirection == EncodingToEncoding) //Between same encodings.
	{
		...
		else //All other encodings
		{
			...
			else
			{
				...
				ret = EntryFunction(fInput, "tempFile", sourceEncoding, "Unicode", TFF, StyleSheetFile);
				if(ret == ConversionDone) 
					ret = EntryFunction("tempFile", fOutput, "Unicode", targetEncoding, TFF, StyleSheetFile);	

				remove("tempFile");//Delete the temproral file.
